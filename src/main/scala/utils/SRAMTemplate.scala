...
class FoldedSRAMTemplate[T <: Data](gen: T, set: Int, width: Int = 4, way: Int = 1,
  ...
  val array = Module(new SRAMTemplate_by(gen, set=nRows, way=width*way,
...
class SRAMTemplateWithArbiter[T <: Data](nRead: Int, gen: T, set: Int, way: Int = 1,
  ...
  val ram = Module(new SRAMTemplate_by(gen, set, way, shouldReset = shouldReset, holdRead = false, singlePort = true))
  ram.io.w <> io.w
